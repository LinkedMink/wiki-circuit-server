...
import { mapToObject } from "../infastructure/CollectionHelpers";
...
export interface IJob {
  progress(value: IProgressModel<unknown>): void;
  result(): unknown | null;
  status(): IJobStatus;
...
export class ArticleJobWork {
  ...
  public doWork = (job: IJob, params: unknown): void => {
    this.job = job;
  ...
  private setProgress = (): void => {
    ...
    if (this.job && total0) {
      this.job.progress({
        ...
        resultSample: mapToObject(this.totals),
      });
    }
  };
